...
class ProformaInvoice(models.Model):
    ...
    party = models.ForeignKey(AddParty, on_delete=models.CASCADE, related_name="proforma_invoices")
    proforma_invoice_number = models.CharField(max_length=20, unique=True, blank=True)  # Auto-generated
    proforma_invoice_date = models.DateField(default=timezone.localdate)
    ...
    sales_person = models.ForeignKey(SalesPerson, on_delete=models.SET_NULL, related_name="proforma_invoices", null=True, blank=True)
    payment_status = models.CharField(max_length=10, choices=PAYMENT_STATUS_CHOICES, default="unpaid")
    status = models.CharField(max_length=30, choices=[("Open", "open"), ("Convert_into_Invoice", "convert_into_invoice")], default="Open")

    def save(self, *args, **kwargs):
        """Ensure proforma_invoice_number is set correctly"""
        if not self.proforma_invoice_number:
            last_invoice = ProformaInvoice.objects.order_by('-id').first()
            next_number = 1 if not last_invoice else int(last_invoice.proforma_invoice_number.split('-')[1]) + 1
            self.proforma_invoice_number = f"PRO-{next_number:05d}"
    ...
    def __str__(self):
        return f"Proforma Invoice {self.proforma_invoice_number} for {self.party_name}"


class ProformaInvoiceItem(models.Model):
    proforma_invoice = models.ForeignKey(ProformaInvoice, on_delete=models.CASCADE, related_name="proforma_invoice_items")
    
    product_item = models.ForeignKey(ProductItem, on_delete=models.SET_NULL, null=True, blank=True, related_name="proforma_invoice_items")
    service_item = models.ForeignKey(ServiceItem, on_delete=models.SET_NULL, null=True, blank=True, related_name="proforma_invoice_services")
...
class ProformaTotalAmount(models.Model):
    proforma_invoice = models.OneToOneField(ProformaInvoice, on_delete=models.CASCADE, related_name="total_amount")
    total = models.DecimalField(max_digits=12, decimal_places=2, blank=True, null=True)

    def save(self, *args, **kwargs):
        """Calculate and store the total for the related ProformaInvoice"""
        if self.proforma_invoice:
            self.total = self.proforma_invoice.proforma_invoice_items.aggregate(models.Sum("amount"))["amount__sum"] or 0.00
        
        super().save(*args, **kwargs)

    def __str__(self):
        return f"Total Amount for {self.proforma_invoice.proforma_invoice_number}: {self.total}"

class ProformaInvoiceEmailLog(models.Model):
    proforma_invoice = models.ForeignKey('ProformaInvoice', on_delete=models.CASCADE, related_name="email_logs")
    ...
    def __str__(self):
        return f"Email to {self.email} - {self.proforma_invoice.proforma_invoice_number}"
